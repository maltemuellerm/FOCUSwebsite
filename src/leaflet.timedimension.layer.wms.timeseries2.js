...
L.TimeDimension.Layer.WMS.TimeSeries2 = L.TimeDimension.Layer.WMS.extend({
    ...
    initialize: function(layer, options) {
        ...
        this._defaultRangeSelector = options.defaultRangeSelector || 4;
    ...
    addPositionMarker: function(point) {
        ...
        var afterLoadData = function(color, data) {
            ...
            if (this._chart2) {
                this._chart2.hideLoading();
            }
        };
        if (this._chart2) {
            this._chart2.showLoading();
        }
    ...
    _checkLoadNewData: function(min, max) {
        ...
        if (min < this._currentDateRange.min) {
            ...
            this._chart2.showLoading();
            for (i = 0, l = this._chart2.series.length; i < l; i++) {
                serie = this._chart2.series[i];
    ...
    _createChart: function() {
        ...
        this._chart2 = Highcharts.stockChart(chart_container, combinedHighChartsOptions);
        this._timeDimension.on('timeload', (function(data) {
            if (!this._chart2){
                return;
            }
            this._chart2.xAxis[0].removePlotBand("pbCurrentTime");
            this._chart2.xAxis[0].addPlotLine({
                color: 'red',
                dashStyle: 'solid',
                value: new Date(this._timeDimension.getCurrentTime()),
                width: 2,
...
L.timeDimension.layer.wms.timeseries2 = function(layer, options) {
    return new L.TimeDimension.Layer.WMS.TimeSeries2(layer, options);
};
